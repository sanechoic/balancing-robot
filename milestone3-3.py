...
1.Use the adapted beat detect from Milestone2 to flash the blue LED every time a beat is detected.
2.As well as flashing the LED, a .txt file containing dance moves (dance moves defined as A,B,C ... F)
is read one line at a time for every beat detected.
3.The dancemoves correspond to the music and routine of 'Strip the Willow'
4. 6 Dance moves have been written, these control the speeds and directions of motors A and B.
'''
...
def StepFunctions(steps, i, v):
	...
	if step == 'A':
		...
		print('Move forward to the right 45')
		motor.right_forward(v / 3)
		motor.left_forward(v)

	elif step == 'C':
		print('Move forward to the left 45')
		motor.right_forward(v)
		motor.left_forward(v / 3)

	elif step == 'D':
		print('Move backwards to the Left 45')
		motor.right_back(v)
		motor.left_back(v / 3)

	elif step == 'E':
		print('Move backwards to the right 45 ')
...
counter = 0 #Reading the .txt file from the first line
			...
			if sum_energy > 50000000:
				...
				if (pyb.millis()-tic > 380):	# if more than 500ms since last beat
					...
					if (c>BEAT_THRESHOLD):		# look for a beat
						...
						if counter >= len(DanceMoveList): #Creates a loop so that .txt file repeats
							counter = 0
						else:
							StepFunctions(DanceMoveList, counter, 25)  # from StepFunctions(steps, i, v), 25 = v = PWM motor speed
							counter += 1 #if counter is less than length of .txt file, continue counting through
